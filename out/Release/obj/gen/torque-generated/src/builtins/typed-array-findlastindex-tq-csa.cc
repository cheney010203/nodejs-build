...
TF_BUILTIN(FindLastIndexAllElementsDetachedContinuation, CodeStubAssembler) {
  ...
  if (block1.is_used()) {
    ...
    tmp2 = Convert_Number_Number_0(state_, TNode<Number>{phi_bb1_5});
    tmp3 = Undefined_0(state_);
    tmp4 = CodeStubAssembler(state_).Call(TNode<Context>{parameter0}, TNode<Object>{parameter2}, TNode<Object>{parameter3}, TNode<Object>{tmp3}, TNode<Object>{tmp2}, TNode<Object>{parameter1});
  ...
  if (block6.is_used()) {
    ...
    tmp7 = CodeStubAssembler(state_).NumberSub(TNode<Number>{phi_bb6_5}, TNode<Number>{tmp6});
...
USE(parameter1);
  ...
  if (block11.is_used()) {
    ...
    tmp12 = FindLastIndexAllElements_0(state_, TNode<Context>{parameter0}, TNode<JSTypedArray>{tmp2}, TNode<JSReceiver>{tmp6}, TNode<Object>{tmp11}, &label13, &tmp14);
    ca_.Goto(&block15);
    if (label13.is_used()) {
      ca_.Bind(&label13);
      ca_.Goto(&block16);
  ...
  if (block16.is_used()) {
    ca_.Bind(&block16);
    tmp15 = ca_.CallStub<Number>(Builtins::CallableFor(ca_.isolate(), Builtin::kFindLastIndexAllElementsDetachedContinuation), parameter0, tmp2, tmp6, tmp11, tmp14.value());
